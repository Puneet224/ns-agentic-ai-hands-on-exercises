...
- [10. Scoring rubric and how to win it](#10-scoring-rubric-and-how-to-win-it)
...
Every phase prompt ends with a **stop gate** that forces Copilot to run the
application, test the flows it built, list unresolved issues, append to
...
After each phase:
1. Read `handoff.md` — that is the phase summary
2. Read the generated code — do not accept blindly
3. Run it yourself; do not trust "it works" without seeing output
4. Commit: `git commit -m "Phase N: <what it does>"`
5. Only then move to the next phase

### handoff.md

Created at the repo root by Phase 1, appended to by every phase after. Sections:
what was built · how to run it · verified working · unresolved issues ·
assumptions made · **contract for the next phase**.

That last section is what makes parallel teamwork possible. Person B building the
SDK reads Person A's contract for endpoint shapes instead of asking; Person C
building the dashboard reads it for repository function signatures. Commit it —
it is shared team state, not scratch notes.
...
3. **List unresolved issues.** Anything broken, stubbed, assumed, or deferred.
   Be specific.
4. **Update `handoff.md`** at the repo root. Append a new section — never
   overwrite previous phases. Structure: What was built · How to run it ·
   Verified working · Unresolved issues · Assumptions made · Contract for the
   next phase.
...
# 10. SCORING RUBRIC AND HOW TO WIN IT

| Criteria | Points | What they look for |
|---|---|---|
| Problem Fit & Theme Alignment | 15 | Clear alignment to the problem statement; directly addresses stated pain points and objectives |
| Working Demo & End-to-End Flow | 25 | Functional prototype demonstrated; clear input → processing → actionable output journey |
| Technical Excellence | 25 | Code quality (5), architecture and design (10), security and resilience (10) |
| Business Impact & Viability | 10 | Clear value proposition, measurable benefits, feasible enterprise adoption |
| Scalability & Production Readiness | 15 | Same |
| Innovation & Differentiation | 10 | Novel approach beyond a generic solution; effective use of AI / automation |
| **Total** | **100** | |

## Where our points come from

**Problem Fit (15)** — Section 2's coverage map traces every line of the
challenge slide to a phase. Put that table in the deck. Do not make judges infer
the mapping; show it.

**Working Demo (25)** — Phase 5 is not optional. A quarter of the total score
depends on a prototype running live. Rehearse the demo end to end at least twice
before presenting, on a machine that is not the one you built on.

**Technical Excellence (25)** — the repository layer covers architecture, the
fail-safe design and bearer auth cover security and resilience, and the pytest
suite covers quality. Show the passing test output on screen; a green terminal
is worth more than a claim.

**Business Impact (10)** — three-line integration is the argument. Cost of
adoption is lower than the cost of debating it.

**Scalability (15)** — repository layer for the PostgreSQL path, offline spool
for outages, WAL mode, batching, bounded queues. Section 9 has the answers.

**Innovation (10) — the one at risk.** The criterion says "effective use of
AI / automation aligned to the problem." We deliberately cut LLMs for time and
reliability, so a judge reading this literally could mark us down. Automation
counts, but only if we name it. Three layers to surface explicitly:

1. **Zero-touch instrumentation** — the decorator captures latency, success or
   failure, error category, and version attribution with no developer effort.
   Compare against manual logging, which is what teams do today.
2. **Autonomous feedback triggering** — no scheduler, no survey campaign, no
   coordinator. The system decides when and whom to ask, per user, per agent.
3. **Adaptive questioning** — the checkup branches on answers rather than
   marching through a fixed form. Low sentiment digs into the failure; high time
   saved captures the win; disengaged users get a short path out.
4. **Automated anomaly detection** — dark agents, version regressions, sentiment
   drops, and latency spikes all surface without anyone querying for them.

Deck line: *"Three layers of automation replace what is manual work today —
instrumentation, feedback collection, and anomaly detection."*

## Note on the rubric itself

The "what to look for" text is identical for Business Impact and Scalability —
almost certainly a copy-paste error in the source document. We cover both
substantively, so it costs us nothing, but do not be thrown if a judge asks the
same question twice.

## Demo run sheet

Rehearse this sequence. Target six minutes.

| Beat | Time | What you show |
|---|---|---|
| Problem | 0:30 | Agents ship into a void — nobody knows who uses them |
| Integration | 1:00 | Open a fresh agent repo, add three lines, run one query |
| Live signal | 0:30 | Event appears on the dashboard in real time |
| Adoption view | 1:00 | Dark agents panel, adoption by Division |
| Feedback view | 1:00 | Day-14 checkup fires, adaptive follow-up, response lands |
| Reliability view | 0:45 | Version comparison showing the regression |
| Resilience | 0:45 | Kill the API, agent keeps working, restart, backlog drains |
| Roadmap | 0:30 | Cost attribution, then degradation monitoring on the same stream |

If you are short on time, cut the resilience beat — not the integration beat.
